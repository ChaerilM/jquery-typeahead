/*!
 * jQuery Typeahead
 * Copyright (C) 2023 RunningCoder.org
  ...
  Typeahead.prototype = {
      ...
      this.node.off(this.namespace).on(events.join(" "), function (e, data) {
        switch (e.type) {
          ...
          case "search":
            ...
            if (
              scope.result.length ||
              (scope.searchGroups.length && scope.displayEmptyTemplate)
            ) {
              scope.showLayout();
            } else {
              scope.hideLayout();
    ...
    generateSource: function (generateGroups) {
      ...
      if (
        ...
        this.searchGroups.length !== this.generateGroups
      ) {
        this.node.trigger("search" + this.namespace);
      }
        ...
        (function (i, item, liHtml) {
          liHtml.on("click", function (e, originalEvent) {
            ...
            if (scope.options.multiselect) {
              ...
              scope.query = scope.rawQuery = scope.getTemplateValue.call(
                scope,
                item,
              );
              if (scope.isContentEditable) {
